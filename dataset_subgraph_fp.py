...
def read_smiles(data_path = './data/pretrain_data/MACCS_data/data10m_maccs'):
    """
    ...
    for i, smiles in enumerate(smiles_data):
        ...
        print(f'smiles: {i + 1}/{len(smiles_data)}')
    maccs_labels = np.array(maccs_labels)

    return smiles_data, labels_1, labels_2, labels_3, maccs_labels
            # mol = Chem.MolFromSmiles(smiles)
            # if mol != None:
    ...
    fp_list = []
    for i in range(80):
        pretrain_data = np.load(data_path + '_{}.npy'.format(i+1), allow_pickle=True)
        smiles_list = smiles_list + [x for x in pretrain_data[0]]
        k100_list = k100_list + [x for x in pretrain_data[1]]
        k1000_list = k1000_list + [x for x in pretrain_data[2]]
        k10000_list = k10000_list + [x for x in pretrain_data[3]]
        fp_list = fp_list + [x for x in pretrain_data[4]]
        print(data_path + '_{}.npy'.format(i + 1) + ' is loaded')
    return smiles_list, k100_list, k1000_list, k10000_list, fp_list
...
class MoleculeDataset(Dataset):
    def __init__(self, data_path):
        ...
        self.smiles_data, self.label1, self.label2, self.label3, self.fp_labels = read_smiles(data_path)
    ...
    def __getitem__(self, index):
        ...
        y_fp = torch.tensor(self.fp_labels[index], dtype=torch.int64)

        data_i = Data(x=x_i, edge_index=edge_index_i, edge_attr=edge_attr_i)
        data_j = Data(x=x_j, edge_index=edge_index_j, edge_attr=edge_attr_j)
        data = Data(x=x, edge_index=edge_index, edge_attr=edge_attr, y1=y1, y2=y2, y3=y3, y_fp=y_fp)
...
def build_maccs_pretrain_data_and_save(smiles_list, k100_list, k1000_list, k10000_list, save_path, global_feature='MACCS'):
...
def build_pubchem_pretrain_data_and_save(smiles_list, k100_list, k1000_list, k10000_list, save_path, global_feature='PubChem'):
    smiles_list = smiles_list
    k100_list = k100_list
    k1000_list = k1000_list
    k10000_list = k10000_list
...
def build_morgan_pretrain_data_and_save(smiles_list, k100_list, k1000_list, k10000_list, save_path, global_feature='morgan'):
    smiles_list = smiles_list
    k100_list = k100_list
    k1000_list = k1000_list
    k10000_list = k10000_list
